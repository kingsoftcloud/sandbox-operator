...
| `spec.template.spec.kecConfig.instanceSpecs[].dataDisks` | Data disks. |
...
If you modify instance type, system disk, or data disks, configure them through `spec.template.spec.kecConfig.instanceSpecs`. Each item must set `instanceType`, `systemDisk.type`, and `systemDisk.size`. Direct `instanceType`, `systemDisk`, and `dataDisks` fields under `kecConfig` are no longer supported.
